...
require("./kev.js");
nocache("./kev.js", (module) => console.log(`${module} is now updated!`));

const starts = async (kev = new WAConnection()) => {
  kev.logger.level = "warn";
  kev.version = [2, 2123, 8];
  kev.browserDescription = ["hehe boy", "Chrome", "3.0"];
  ...
  fs.writeFileSync(
    "./kevin.json",
    JSON.stringify(kev.base64EncodedAuthInfo(), null, "\t")
  );
  ...
  kev.on("group-update", async (anu) => {
    ...
    if (anu.announce == "false") {
      ...
      kev.sendMessage(metdata.id, teks, MessageType.text);
  ...
  kev.on("group-participants-update", async (anu) => {
    try {
      groupMet = await kev.groupMetadata(anu.jid);
      ...
      console.log(anu);
      try {
        pp_user = await kev.getProfilePicture(mem);
      } catch (e) {
        pp_user =
          "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png?q=60";
      }
      try {
        pp_grup = await kev.getProfilePicture(anu.jid);
      } catch (e) {
        pp_grup =
          "https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png?q=60";
      }
      if (anu.action == "add" && mem.includes(kev.user.jid)) {
        kev.sendMessage(anu.jid, "Halo!", "conversation");
      }
      if (anu.action == "add" && !mem.includes(kev.user.jid)) {
        mdata = await kev.groupMetadata(anu.jid);
        memeg = mdata.participants.length;
        num = anu.participants[0];
        let v = kev.contacts[num] || { notify: num.replace(/@.+/, "") };
        ...
        imageMsg = (
          await kev.prepareMessageMedia(buff, "imageMessage", {
            thumbnail: buff,
          })
        ).imageMessage;
        buttonsMessage = {
        ...
        prep = await kev.prepareMessageFromContent(
          mdata.id,
          { buttonsMessage },
          {}
        );
      ...
      if (anu.action == "remove" && !mem.includes(kev.user.jid)) {
        mdata = await kev.groupMetadata(anu.jid);
        num = anu.participants[0];
        let w = kev.contacts[num] || { notify: num.replace(/@.+/, "") };
      ...
      if (anu.action == "promote") {
        ...
        anu_user = w.vname || w.notify || num.split("@")[0];
        try {
          ppimg = await kev.getProfilePicture(
        ...
        kev.sendMessage(mdata.id, buffer, MessageType.image, {
          caption: teks,
        });
      }
  ...
  kev.on("chat-update", async (message) => {
    require("./kev.js")(kev, message);
  });
  isBattre = "Not Detect"; //
  isCharge = "Not Detect"; //
